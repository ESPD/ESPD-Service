...
(function ($) {
    ...
    FileInput.prototype = {
        ...
        init: function (options) {
            ...
            if(self.showDownload == true) {
            	self.setCaption(self.downloadTitle, self.downloadUrl, self.hiddenUploadPath, self.hiddenUploadValue);
            	self.$container.removeClass('file-input-new');
            	self.$container.find(".file-input-control").addClass(self.initialFileClass);
            }
        ...
        clear: function (trig) {
            ...
            self.setFileDropZoneTitle();
        	self.$container.find(".file-input-control").removeClass(self.initialFileClass);
        },
        reset: function () {
            var self = this;
            self.clear(true);
        ...
        setCaption: function (content, url, control, value) {
            var self = this, title = $('<div>' + content + '</div>').text(), icon = self.getLayoutTemplate('icon');
            if (self.$caption.length === 0) {
                return;
            }
            var controlTag = '<input type="hidden" name="'+control+'" value="'+value+'"/>';
            self.$caption.html((url) ? (icon + "<a href=\""+url+"\">"+title+"</a>" + controlTag) : title);
    ...
    $.fn.fileinput.defaults = {
    	...
    	showDownload : false,
    	downloadTitle : "no download avaliable",
    	downloadUrl : '#',
    	hiddenUploadPath : '',
    	hiddenUploadValue : false,
    	initialFileClass : '',
