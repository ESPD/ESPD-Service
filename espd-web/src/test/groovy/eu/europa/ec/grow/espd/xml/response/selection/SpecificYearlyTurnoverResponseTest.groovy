 ...
 */
class SpecificYearlyTurnoverResponseTest extends AbstractSelectionCriteriaFixture {
    ...
    def "08. should contain the 'Specific yearly turnover' criterion"() {
        ...
        checkCriterionId(response, idx, "074f6031-55f9-4e99-b9a4-c4363e8bc315")

        then: "CriterionTypeCode element"
        checkCriterionTypeCode(response, idx, "CRITERION.SELECTION.ECONOMIC_FINANCIAL_STANDING.TURNOVER.SPECIFIC_YEARLY")

        then: "CriterionName element"
        response.Criterion[idx].Name.text() == "Specific yearly turnover"

        then: "CriterionDescription element"
        response.Criterion[idx].Description.text() == "Its specific yearly turnover in the business area covered by the contract for the number of financial years required in the relevant notice, the procurement documents or the ESPD is as follows:"
    ...
    def "check the 'Info electronically issuer' requirement response"() {
        given:
        def espd = new EspdDocument(specificYearlyTurnover: new EconomicFinancialStandingCriterion(exists: true,
                availableElectronically: new AvailableElectronically(answer: true, issuer: "HODOR_08")))
        ...
        def req = g6_1.Requirement[2]
        req.Response.size() == 1
        req.Response[0].Description.text() == "HODOR_08"
    }
