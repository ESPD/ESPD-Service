...
class EducationalProfessionalQualificationsResponseTest extends AbstractSelectionCriteriaFixture {
    ...
    def "22. should contain the 'Educational and professional qualifications' criterion"() {
        ...
        then: "main sub group"
        request.Criterion[idx].RequirementGroup[0].ID.text() == "094618e3-b243-49ce-9b12-6aab357f2f88"
    ...
    def "check the 'Info electronically URL' requirement response"() {
        given:
        def espd = new EspdDocument(educationalProfessionalQualifications: new TechnicalProfessionalCriterion(exists: true,
                availableElectronically: new AvailableElectronically(exists: true, url: "http://hodor_22.com")))
        ...
        req.Response.size() == 1
        req.Response[0].Evidence.EvidenceDocumentReference.Attachment.ExternalReference.URI.text() == "http://hodor_22.com"
    }

    def "check the 'Info electronically code' requirement response"() {
        given:
        def espd = new EspdDocument(educationalProfessionalQualifications: new TechnicalProfessionalCriterion(exists: true,
                availableElectronically: new AvailableElectronically(exists: true, code: "HODOR_22")))
